...
class _ChatsPageState extends State<ChatsPage> {
  ...
  Future<void> _loadChats() async {
    ...
    try {
      ...
      //    match → chat → last_message, profile_a, profile_b
      ...
      // 3. Build a flat list with the "other" person's info
      final List<Map<String, dynamic>> chatList = [];
      for (final row in matchRows as List) {
        final chatData = row['chat'];
        if (chatData == null)
          continue; // match without chat room (shouldn't happen)

        // chat is a single object (1-to-1 via UNIQUE match_id)
